...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("BFL")
    updateDisplayUnit();
}
...
{
    QString ret = QString("buffalocoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BFL unit input in lnReqAmount, we generate the URI with BFL as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BFL, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
